...
-- Tambahkan fungsi ini untuk menekan UI Skill Asli di PlayerGui
local function PressInGameSkill(key)
    ...
    if Skills then
        for _, f in pairs(Skills:GetChildren()) do
            if f:IsA("Frame") and f.Visible and f:FindFirstChild(key) then
                ...
                -- Tekan UI Asli agar game merespon (muncul aim/indikator)
                FireUI(btn) 
                return true
            end
        end
    ...
    if Ctx then
        for _, f in pairs(Ctx:GetChildren()) do
            if f.Name:find("Dodge") then
                ...
                if btn then FireUI(btn) return end
            end
        end
    end
end
...
-- ==============================================================================
-- [3] COMBAT LOGIC (FIXED & DEFINED)
-- ==============================================================================

-- 1. FUNGSI UTAMA: Mengatur Status Tekanan Skill (PENTING untuk Hold/Release)
local function setSkillState(keyName, isDown)
    local PGui = LocalPlayer:FindFirstChild("PlayerGui")
    local Skills = PGui and PGui:FindFirstChild("Main") and PGui.Main:FindFirstChild("Skills")
    
    if Skills then
        for _, f in pairs(Skills:GetChildren()) do
            -- Mencari tombol skill (Z, X, C, V, F) yang sesuai dan sedang terlihat
            if f:IsA("Frame") and f.Visible and f:FindFirstChild(keyName) then
                local btn = f[keyName]:FindFirstChild("Mobile") or f[keyName]
                
                if btn and getconnections then
                    if isDown then
                        -- SIMULASI JARI MENEMPEL (Mulai Charge/Hold)
                        for _, c in pairs(getconnections(btn.InputBegan)) do
                            c:Fire({UserInputType=Enum.UserInputType.Touch, UserInputState=Enum.UserInputState.Begin})
                        end
                    else
                        -- SIMULASI JARI DIANGKAT (Tembak Skill)
                        ...
                        -- Activated sebagai backup agar skill pasti keluar
                        for _, c in pairs(getconnections(btn.Activated)) do c:Fire() end
                    end
                end
                return
...
-- 2. FUNGSI pressKey: Diperbarui agar menggunakan logic setSkillState (Tap Cepat)
local function pressKey(keyName)
    setSkillState(keyName, true)  -- Tekan
    task.wait(0.05)               -- Jeda sangat singkat (registering)
    setSkillState(keyName, false) -- Lepas
end

-- 3. FUNGSI isWeaponReady: Tetap (Sudah Benar)
...
-- 4. FUNGSI equipWeapon: Mendukung Klik 1x Pasang, Klik 2x Lepas (Toggle)
local function equipWeapon(slotIdx, isToggle)
    ...
    -- Logika Toggle: Jika sedang pegang senjata yang sama
    if currentTool and currentTool.ToolTip == targetTooltip then
        if isToggle then
            hum:UnequipTools() -- Lepas senjata (Klik kedua kali)
        end
        return -- Selesai
    end
    
    -- Logika Equip: Jika belum pegang, cari di Backpack
    ...
    if toolToEquip then 
        ...
        task.wait(0.1) -- Jeda animasi equip agar tidak bug
    end
end


-- ==============================================================================
-- [4] INPUT HANDLERS (SMART AIM & CHARGE FIX)
-- ==============================================================================

local IsSmartHolding = false 
local SmartTouchObject = nil 

-- Helper untuk mengirim input sentuhan virtual
local function SendTouchAt(x, y, state)
    -- state 0 = Begin (Down), 2 = End (Up), 1 = Move
    VIM:SendTouchEvent(5, state, x, y)
end
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if (input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1) then
        ...
        -- A. TRIGGER SKILL SMART (Charge/Hold Logic)
        if SkillMode == "SMART" and CurrentSmartKeyData ~= nil then
            SmartTouchObject = input -- Simpan input jari ini
            IsSmartHolding = true
            
            -- Ambil posisi jari user
            local x, y = input.Position.X, input.Position.Y
            
            -- KIRIM SINYAL TOUCH KE GAME (Simulasi jari menekan skill di dunia game)
            -- Karena UI Skill sudah ditekan sebelumnya (di toggleVirtualKey),
            -- sekarang game menunggu input sentuhan di layar untuk mengarahkan/menembak.
            SendTouchAt(x, y, 0) -- 0 = Touch Down (Mulai Charge/Aim)
            
            return -- Stop agar tidak bentrok
        end
...
UserInputService.InputEnded:Connect(function(input)
    -- C. LEPAS SKILL SMART (Fire Logic)
    if input == SmartTouchObject and SkillMode == "SMART" and CurrentSmartKeyData ~= nil then
        
        -- Ambil posisi jari terakhir saat dilepas
        local x, y = input.Position.X, input.Position.Y
        
        -- KIRIM SINYAL LEPAS TOUCH (Game akan menembakkan skill)
        SendTouchAt(x, y, 2) -- 2 = Touch Up (Fire!)
        
        -- Reset Visual Tombol Script
...
-- Opsional: Tambahkan ini agar Aim bisa digerakkan (Drag Aim)
UserInputService.InputChanged:Connect(function(input)
    if input == SmartTouchObject and IsSmartHolding and SkillMode == "SMART" then
        local x, y = input.Position.X, input.Position.Y
        SendTouchAt(x, y, 1) -- 1 = Touch Move (Update Arah Aim)
    end
end)
...
UserInputService.InputEnded:Connect(function(input)
    ...
    if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
        IsSmartHolding = false -- Lepas flag hold agar combo lanjut ke step 2
    end

    -- Reset Smart Skill (Single)
    if input == SmartTouchObject and SkillMode == "SMART" and CurrentSmartKeyData ~= nil then
        if ActiveVirtualKeys[CurrentSmartKeyData.ID] then
            local btn = ActiveVirtualKeys[CurrentSmartKeyData.ID].Button
...
local GText = [[WELCOME TO VELOX ! (MOBILE EDITION)
...
[ SETTINGS ]
Go to 'LAYOUT' tab to enable Joystick, Change Transparency, or Change Skill Mode.
            ...
            if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                if isSkillKey then
                    if SkillMode == "INSTANT" then
                        -- Mode Instant: Langsung tembak (Tekan UI + Tekan Layar)
                        PressInGameSkill(kn) -- Tekan UI
                        task.wait(0.01)
                        local vp = Camera.ViewportSize
                    ...
                    elseif SkillMode == "SMART" then
                        -- [PERBAIKAN LOGIKA SMART]
                        if CurrentSmartKeyData and CurrentSmartKeyData.ID == id then
                            -- Jika ditekan lagi -> Cancel (Disarm)
                            CurrentSmartKeyData = nil
                            btn.BackgroundColor3 = Color3.fromRGB(0,0,0); btn.TextColor3 = Theme.Accent
                        else
                            -- 1. Reset tombol sebelumnya jika ada
                            ...
                            -- 2. Equip Senjata dulu
                            if slotIdx then equipWeapon(slotIdx, false) end
                            
                            -- 3. TEKAN UI GAME SEKARANG! (Agar indikator skill muncul)
                            PressInGameSkill(kn) 
                            
                            -- 4. Set Status "Siap Tembak"
                            CurrentSmartKeyData = vData
                            
                            -- 5. Ubah Visual Tombol jadi Hijau
...
CreateComboButtonFunc = function(idx, loadedSteps)
    ...
    btn.Position = UDim2.new(0.5, -30, 0.5, 0)
    ...
    btn.Selectable = false
    createCorner(btn, 30)
    local st = createStroke(btn, Theme.Accent)
    btn.ZIndex = 70
