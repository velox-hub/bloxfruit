...
local function toggleVirtualKey(keyName, slotIdx, customName)
    ...
    if ActiveVirtualKeys[id] then 
        ...
        -- Hapus InputEnded karena kita tidak pakai hold key lagi untuk skill
...
RefreshEditorUI = function()
    ...
    for i,s in ipairs(d.Steps) do
        ...
        mkSlid(30, "Delay: +%.1fs", s.Delay or 0, 2.0, function(p) s.Delay=math.floor(p*2*10)/10 end, Theme.SubText)
        -- Keep Wait After Cast slider
        mkSlid(55, "Wait: %.1fs", s.HoldTime or 0.1, 2.0, function(p) s.HoldTime=math.floor(p*2*10)/10 end, Theme.Accent)
    end
end
...
local ModeBox = Instance.new("Frame"); ModeBox.Size=UDim2.new(0.95,0,0,50); ModeBox.BackgroundColor3=Theme.Sidebar; ModeBox.LayoutOrder=1; ModeBox.Parent=P_Control; createCorner(ModeBox,6)
...
local function LoadSpecific(configName)
    ...
    if data.Keybinds then
        for id, keyName in pairs(data.Keybinds) do
            if Enum.KeyCode[keyName] then Keybinds[id] = Enum.KeyCode[keyName] end
        end
    end
