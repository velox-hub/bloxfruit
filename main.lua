...
-- === PRE-DECLARATION ===
local JoyOuter, JoyKnob, JoyDrag, ToggleBtn, JoyContainer, LockBtn, HideBtn
...
UpdateTransparencyFunc = function()
    ...
    if ToggleBtn then 
        ...
        if ToggleBtn:FindFirstChild("UIStroke") then ToggleBtn.UIStroke.Transparency = sT end
    end
    for _, c in pairs(Combos) do 
        if c.Button then 
            c.Button.BackgroundTransparency = t
            c.Button.TextTransparency = t
            if c.Button:FindFirstChild("UIStroke") then c.Button.UIStroke.Transparency = sT end
        end 
    end
    for _, item in pairs(ActiveVirtualKeys) do 
        ...
        if btn:FindFirstChild("UIStroke") then btn.UIStroke.Transparency = sT end
    end
...
-- JOYSTICK UI (World Movement + Input Sink + Pause Logic)
...
local TaglineText = Instance.new("TextLabel"); TaglineText.Size=UDim2.new(1,0,0,15); TaglineText.Position=UDim2.new(0,0,0.6,0); TaglineText.Text="Total Control. Instant Combo."; TaglineText.TextColor3=Theme.SubText; TaglineText.Font=Enum.Font.Gotham; TaglineText.TextSize=9; TaglineText.BackgroundTransparency=1; TaglineText.Parent=BrandFrame
...
local PageTitle = Instance.new("TextLabel"); PageTitle.Size=UDim2.new(1,0,0,30); PageTitle.Text="GUIDE & INFO"; PageTitle.Font=Enum.Font.GothamBlack; PageTitle.TextSize=22; PageTitle.TextColor3=Theme.Text; PageTitle.TextXAlignment="Left"; PageTitle.BackgroundTransparency=1; PageTitle.Parent=Content
...
local function mkNav(icon, text, target, title)
    ...
    btn.MouseButton1Click:Connect(function() nav(target, title); for _, c in pairs(NavContainer:GetChildren()) do if c:IsA("TextButton") then c.TextColor3=Theme.SubText; c.BackgroundColor3=Theme.Bg; c.BackgroundTransparency=1 end end; btn.TextColor3=Theme.Accent; btn.BackgroundColor3=Theme.Element; btn.BackgroundTransparency=0 end)
    return btn
end

local P_Guide = Instance.new("Frame"); P_Guide.Size=UDim2.new(1,0,0.85,0); P_Guide.Position=UDim2.new(0,0,0.15,0); P_Guide.BackgroundTransparency=1; P_Guide.Parent=Content; Pages["Guide"]=P_Guide
local P_Edit = Instance.new("Frame"); P_Edit.Size=UDim2.new(1,0,0.85,0); P_Edit.Position=UDim2.new(0,0,0.15,0); P_Edit.BackgroundTransparency=1; P_Edit.Parent=Content; Pages["Editor"]=P_Edit
...
local NavGuide=mkNav("ℹ️","Guide","Guide","GUIDE & INFO"); NavGuide.TextColor3=Theme.Accent; NavGuide.BackgroundColor3=Theme.Element; NavGuide.BackgroundTransparency=0
local NavEdit=mkNav("⚔️","Combo","Editor","COMBO EDITOR"); 
local NavCtrl=mkNav("🎮","Controls","Control","CONTROLS");
local NavLay=mkNav("🛠️","Layout","Layout","LAYOUT & SETTINGS"); 
local NavSys=mkNav("⚙️","System","System","SYSTEM & EXIT")

-- STATIC GUIDE
local GFrame = Instance.new("Frame"); GFrame.Size=UDim2.new(1,0,1,0); GFrame.BackgroundTransparency=1; GFrame.Parent=P_Guide
local GText = [[WELCOME TO VELOX!
...
[ JOYSTICK FIX ]
Movement pauses briefly when firing skills to prevent aiming at feet.]]
local GLabel = Instance.new("TextLabel"); GLabel.Size=UDim2.new(0.95,0,0.95,0); GLabel.Position=UDim2.new(0.025,0,0.025,0); GLabel.Text=GText; GLabel.TextColor3=Theme.Text; GLabel.Font=Enum.Font.Gotham; GLabel.TextSize=12; GLabel.TextXAlignment="Left"; GLabel.TextYAlignment="Top"; GLabel.BackgroundTransparency=1; GLabel.TextWrapped=true; GLabel.Parent=GFrame

-- === MANAGER ===
local function toggleVirtualKey(keyName, slotIdx, customName)
    local id = customName or keyName
    if ActiveVirtualKeys[id] then 
        ActiveVirtualKeys[id].Button:Destroy()
        ...
        if ResizerUpdateFunc then ResizerUpdateFunc() end
        if RefreshControlUI then RefreshControlUI() end
    else
        ...
        local kCode; 
        if keyName=="1" then kCode=Enum.KeyCode.One elseif keyName=="2" then kCode=Enum.KeyCode.Two elseif keyName=="3" then kCode=Enum.KeyCode.Three elseif keyName=="4" then kCode=Enum.KeyCode.Four 
...
local TransBox = Instance.new("Frame"); TransBox.Size=UDim2.new(1,0,0,30); TransBox.BackgroundTransparency=1; TransBox.LayoutOrder=1; TransBox.Parent=P_Lay
...
local SetBox = Instance.new("Frame"); SetBox.Size=UDim2.new(1,0,0,115); SetBox.BackgroundTransparency=1; SetBox.LayoutOrder=2; SetBox.Parent=P_Lay
...
JoyToggle.MouseButton1Click:Connect(function() 
    IsJoystickEnabled = not IsJoystickEnabled; JoyContainer.Visible = IsJoystickEnabled; if IsJoystickEnabled then JoyContainer.Position = UDim2.new(0.1, 0, 0.6, 0) end; JoyToggle.Text = IsJoystickEnabled and "JOYSTICK: ON" or "JOYSTICK: OFF"; JoyToggle.BackgroundColor3 = IsJoystickEnabled and Theme.Green or Theme.Red; updateLockState() 
end)
HideBtn = mkTool("HIDE BUTTONS: OFF", Theme.Red, function() AreButtonsHidden=not AreButtonsHidden; HideBtn.Text=AreButtonsHidden and "HIDE BUTTONS: ON" or "HIDE BUTTONS: OFF"; HideBtn.BackgroundColor3=AreButtonsHidden and Theme.Green or Theme.Red; UpdateTransparencyFunc() end, SetBox)
...
mkTool("ADD COMBO", Theme.Accent, function() 
    ...
    MakeDraggable(btn, function() 
        if SkillMode == "INSTANT" then PauseJoystickForAim(); executeComboSequence(idx)
        else
            if SelectedComboID == idx then
                SelectedComboID = nil; btn.BackgroundColor3 = Theme.Sidebar; btn.UIStroke.Color = Theme.Accent; isRunning = false
            else
    ...
    table.insert(Combos, {ID=idx, Name="C"..idx, Button=btn, Steps={}})
    CurrentComboIndex=idx; UpdateTransparencyFunc(); if ResizerUpdateFunc then ResizerUpdateFunc() end; RefreshEditorUI(); ShowNotification("Combo Added", Theme.Green); if RefreshControlUI then RefreshControlUI() end
end, SetBox)

mkTool("DEL COMBO", Theme.Red, function() 
    if #Combos<1 then return end
    Combos[CurrentComboIndex].Button:Destroy()
    table.remove(Combos, CurrentComboIndex)
    if #Combos > 0 then CurrentComboIndex=math.min(CurrentComboIndex, #Combos) else CurrentComboIndex=0 end
    if ResizerUpdateFunc then ResizerUpdateFunc() end; RefreshEditorUI(); ShowNotification("Combo Deleted", Theme.Red); if RefreshControlUI then RefreshControlUI() end
end, SetBox)

-- === V-KEY CREATOR ===
local VKeyLabel = Instance.new("TextLabel"); VKeyLabel.Size=UDim2.new(1,0,0,20); VKeyLabel.Text="ADD VIRTUAL KEY"; VKeyLabel.TextColor3=Theme.Accent; VKeyLabel.Font=Enum.Font.GothamBold; VKeyLabel.TextSize=11; VKeyLabel.BackgroundTransparency=1; VKeyLabel.LayoutOrder=3; VKeyLabel.Parent=P_Lay
local VKeyBox = Instance.new("Frame"); VKeyBox.Size=UDim2.new(1,0,0,80); VKeyBox.BackgroundColor3=Theme.Sidebar; VKeyBox.LayoutOrder=4; VKeyBox.Parent=P_Lay; createCorner(VKeyBox,6)
local SelType = "Melee"; local SelKey = "Z"
local TypeBtn = Instance.new("TextButton"); TypeBtn.Size=UDim2.new(0.45,0,0,30); TypeBtn.Position=UDim2.new(0.03,0,0.1,0); TypeBtn.BackgroundColor3=Theme.Element; TypeBtn.Text=SelType; TypeBtn.TextColor3=Theme.Text; TypeBtn.Parent=VKeyBox; createCorner(TypeBtn,6)
local KeyBtn = Instance.new("TextButton"); KeyBtn.Size=UDim2.new(0.45,0,0,30); KeyBtn.Position=UDim2.new(0.52,0,0.1,0); KeyBtn.BackgroundColor3=Theme.Element; KeyBtn.Text=SelKey; KeyBtn.TextColor3=Theme.Text; KeyBtn.Parent=VKeyBox; createCorner(KeyBtn,6)
local AddVKeyBtn = Instance.new("TextButton"); AddVKeyBtn.Size=UDim2.new(0.94,0,0,30); AddVKeyBtn.Position=UDim2.new(0.03,0,0.55,0); AddVKeyBtn.BackgroundColor3=Theme.Green; AddVKeyBtn.Text="ADD BUTTON"; AddVKeyBtn.TextColor3=Theme.Bg; AddVKeyBtn.Font=Enum.Font.GothamBold; AddVKeyBtn.Parent=VKeyBox; createCorner(AddVKeyBtn,6)

TypeBtn.MouseButton1Click:Connect(function()
    local tList = {"Melee", "Fruit", "Sword", "Gun"}
    local idx = table.find(tList, SelType) or 0; SelType = tList[(idx % #tList) + 1]; TypeBtn.Text = SelType
end)
KeyBtn.MouseButton1Click:Connect(function()
    local kList = {"Z", "X", "C", "V", "F"}
    local idx = table.find(kList, SelKey) or 0; SelKey = kList[(idx % #kList) + 1]; KeyBtn.Text = SelKey
end)
AddVKeyBtn.MouseButton1Click:Connect(function()
    local slot = 1; for i,v in ipairs(WeaponData) do if v.name == SelType then slot = i break end end
    local id = SelType.." "..SelKey
    toggleVirtualKey(SelKey, slot, id)
end)
...
ModeBtn.MouseButton1Click:Connect(function()
    if SkillMode == "INSTANT" then
        SkillMode = "SMART"; ModeBtn.Text = "SKILL MODE: SMART TAP"; ModeBtn.BackgroundColor3 = Theme.Blue
    else
        SkillMode = "INSTANT"; ModeBtn.Text = "SKILL MODE: INSTANT"; ModeBtn.BackgroundColor3 = Theme.Green; CurrentSmartKeyData=nil; SelectedComboID=nil
    end
end)

local BindContainer = Instance.new("Frame"); BindContainer.AutomaticSize=Enum.AutomaticSize.Y; BindContainer.Size=UDim2.new(1,0,0,0); BindContainer.BackgroundTransparency=1; BindContainer.LayoutOrder=2; BindContainer.Parent=P_Control
local BindList = Instance.new("UIListLayout"); BindList.Parent=BindContainer; BindList.Padding=UDim.new(0,5)
...
RefreshControlUI = function()
    ...
    for _, c in ipairs(Combos) do
        ...
        local bindKey = nil; for k,v in pairs(Keybinds) do if k == "C"..c.ID then bindKey=v end end
        b.Text = bindKey and bindKey.Name or "BIND"
        b.MouseButton1Click:Connect(function()
            b.Text="..."
            local conn; conn = UserInputService.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Keyboard then
                    Keybinds["C"..c.ID] = input.KeyCode; b.Text=input.KeyCode.Name; conn:Disconnect()
                end
            end)
        end)
    end
    ...
    for id, vData in pairs(ActiveVirtualKeys) do
        ...
        b.MouseButton1Click:Connect(function()
            ...
            local conn; conn = UserInputService.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.Keyboard then
                    Keybinds[id] = input.KeyCode; b.Text=input.KeyCode.Name; conn:Disconnect()
                end
            end)
        end)
    end
...
-- === RESIZER UI ===
local AdvLabel = Instance.new("TextLabel"); AdvLabel.Size=UDim2.new(1,0,0,20); AdvLabel.Text="RESIZE BUTTON"; AdvLabel.TextColor3=Theme.Accent; AdvLabel.Font=Enum.Font.GothamBold; AdvLabel.TextSize=11; AdvLabel.BackgroundTransparency=1; AdvLabel.LayoutOrder=5; AdvLabel.Parent=P_Lay
local AdvBox = Instance.new("Frame"); AdvBox.Size=UDim2.new(1,0,0,90); AdvBox.BackgroundColor3=Theme.Sidebar; AdvBox.LayoutOrder=6; AdvBox.Parent=P_Lay; createCorner(AdvBox,6)
...
local SizeSlider = Instance.new("Frame"); SizeSlider.Size=UDim2.new(0.9,0,0,4); SizeSlider.Position=UDim2.new(0.05,0,0.7,0); SizeSlider.BackgroundColor3=Theme.Stroke; SizeSlider.Parent=AdvBox; createCorner(SizeSlider,2)
...
ResizerUpdateFunc = function()
    ...
    if #ResizerList == 0 then SelectBtn.Text = "SELECT: NONE"; CurrentSelectedElement = nil; return end
...
-- === UPDATE HOOKS ===
Add.MouseButton1Click:Connect(function() table.insert(Combos[CurrentComboIndex].Steps, {Slot=1, Key="Z", Delay=0, IsHold=false, HoldTime=0.5}); RefreshEditorUI() end)
RunService.Heartbeat:Connect(function() 
    if P_Edit.Visible then
        if CurrentComboIndex > 0 then EmptyMsg.Visible=false; Scroll.Visible=true; BottomBar.Visible=true; TopNav.Visible=true
        else EmptyMsg.Visible=true; Scroll.Visible=false; BottomBar.Visible=false; TopNav.Visible=false end 
    else EmptyMsg.Visible=false end
end)
...
local function GetCurrentState()
    local vkData = {}
    for id, vData in pairs(ActiveVirtualKeys) do 
        table.insert(vkData, {ID=id, Key=vData.Key.Name, Slot=vData.Slot, Pos={vData.Button.Position.X.Scale, vData.Button.Position.X.Offset, vData.Button.Position.Y.Scale, vData.Button.Position.Y.Offset}, Size=vData.Button.Size.X.Offset}) 
    end
    local comboData = {}
    for _, c in ipairs(Combos) do table.insert(comboData, {ID = c.ID, Name=c.Name, Steps = c.Steps, Pos = {c.Button.Position.X.Scale, c.Button.Position.X.Offset, c.Button.Position.Y.Scale, c.Button.Position.Y.Offset}, Size=c.Button.Size.X.Offset}) end
    local togglePos = {ToggleBtn.Position.X.Scale, ToggleBtn.Position.X.Offset, ToggleBtn.Position.Y.Scale, ToggleBtn.Position.Y.Offset}
    
    -- Serialize Keybinds
    local kbData = {}
    for k, v in pairs(Keybinds) do kbData[k] = v.Name end

    return {
        LayoutLocked = IsLayoutLocked,
        Combos = comboData, 
        Layout = {Transparency = GlobalTransparency, JoyPos = {JoyContainer.Position.X.Offset, JoyContainer.Position.Y.Offset}, JoySize=JoyOuter.Size.X.Offset, JoyOn = IsJoystickEnabled, TogglePos = togglePos, ButtonsHidden = AreButtonsHidden},
        VKeys = vkData,
        Keybinds = kbData,
        SkillMode = SkillMode
    }
...
local function SaveToFile(name, data)
    local success, err = pcall(function()
        ...
        if isfile(FileName) then
            local r = readfile(FileName); if r then allConfigs = HttpService:JSONDecode(r) end
        end
        allConfigs[name] = data
        allConfigs["LastUsed"] = name
        writefile(FileName, HttpService:JSONEncode(allConfigs))
    end)
    if success then CurrentConfigName = name; ShowNotification("Saved: " .. name, Theme.Green) else warn("Save Error: ", err) end
end

local function LoadSpecific(name)
    local success, err = pcall(function()
        if isfile(FileName) then
            local r = readfile(FileName)
            local all = HttpService:JSONDecode(r)
            local data = all[name]
            if data then
                ...
                GlobalTransparency = data.Layout.Transparency or 0
                AreButtonsHidden = data.Layout.ButtonsHidden or false
                HideBtn.Text = AreButtonsHidden and "HIDE BUTTONS: ON" or "HIDE BUTTONS: OFF"
                HideBtn.BackgroundColor3 = AreButtonsHidden and Theme.Green or Theme.Red
                
                TKnob.Position = UDim2.new(GlobalTransparency/0.9, -6, 0.5, -6)
                if data.LayoutLocked ~= nil then IsLayoutLocked = data.LayoutLocked end
                if data.Layout.JoyPos then JoyContainer.Position = UDim2.new(0, data.Layout.JoyPos[1], 0, data.Layout.JoyPos[2]) end
                if data.Layout.JoySize then local s = data.Layout.JoySize; JoyOuter.Size=UDim2.new(0,s,0,s); JoyContainer.Size=UDim2.new(0,s,0,s+30); createCorner(JoyOuter,s) end
                if data.Layout.JoyOn ~= nil then 
                    IsJoystickEnabled = data.Layout.JoyOn; JoyContainer.Visible = IsJoystickEnabled
                    JoyToggle.Text = IsJoystickEnabled and "JOYSTICK: ON" or "JOYSTICK: OFF"; JoyToggle.BackgroundColor3 = IsJoystickEnabled and Theme.Green or Theme.Red
                end
                if data.Layout.TogglePos then ToggleBtn.Position = UDim2.new(data.Layout.TogglePos[1], data.Layout.TogglePos[2], data.Layout.TogglePos[3], data.Layout.TogglePos[4]) end
                
                -- Restore VKeys
                for _, vk in ipairs(data.VKeys or {}) do 
                    toggleVirtualKey(vk.Key, vk.Slot, vk.ID) -- Corrected args
                    local vData = ActiveVirtualKeys[vk.ID]
                    if vData then
                        vData.Button.Position = UDim2.new(vk.Pos[1], vk.Pos[2], vk.Pos[3], vk.Pos[4]) 
                        if vk.Size then vData.Button.Size = UDim2.new(0,vk.Size,0,vk.Size) end
                    end
                end
                ...
                for _, cData in ipairs(data.Combos) do
                    local idx = #Combos + 1
                    local btn = Instance.new("TextButton"); local s = cData.Size or 60; btn.Size=UDim2.new(0,s,0,s); btn.Position=UDim2.new(cData.Pos[1], cData.Pos[2], cData.Pos[3], cData.Pos[4]); btn.BackgroundColor3=Theme.Sidebar; btn.Text=cData.Name or ("C"..idx); btn.TextColor3=Theme.Accent; btn.Font=Enum.Font.GothamBold; btn.TextSize=18; btn.Parent=ScreenGui; btn.Selectable=false; createCorner(btn,s/2); local st=createStroke(btn,Theme.Accent); btn.ZIndex=70
                    MakeDraggable(btn, function() 
                        if SkillMode == "INSTANT" then PauseJoystickForAim(); executeComboSequence(idx)
                        else
                    ...
                    table.insert(Combos, {ID=idx, Name=cData.Name or ("C"..idx), Button=btn, Steps=cData.Steps})
                end
                
                -- Restore System Settings
                if data.SkillMode then 
                    SkillMode = data.SkillMode
                    ModeBtn.Text = SkillMode == "INSTANT" and "SKILL MODE: INSTANT" or "SKILL MODE: SMART TAP"
                    ModeBtn.BackgroundColor3 = SkillMode == "INSTANT" and Theme.Green or Theme.Blue
                end
                
                -- Restore Keybinds
                Keybinds = {}
                if data.Keybinds then
                    for k, v in pairs(data.Keybinds) do Keybinds[k] = Enum.KeyCode[v] end
                end
                
                if #Combos > 0 then CurrentComboIndex = 1; RefreshEditorUI() else CurrentComboIndex=0 end
                if ResizerUpdateFunc then ResizerUpdateFunc() end 
                UpdateTransparencyFunc()
                updateLockState()
                ...
                all["LastUsed"] = name
                writefile(FileName, HttpService:JSONEncode(all))
            end
        end
    end)
    if success then ShowNotification("Loaded: " .. name, Theme.Green) else warn("Load Error: ", err) end
end
...
local SaveBtn = mkTool("SAVE CONFIG", Theme.Blue, function()
    if CurrentConfigName then
        ShowPopup("SAVE OPTIONS", function(c)
            local b1 = Instance.new("TextButton"); b1.Size=UDim2.new(1,0,0,35); b1.BackgroundColor3=Theme.Bg; b1.Text="Overwrite '"..CurrentConfigName.."'"; b1.TextColor3=Theme.Accent; b1.Parent=c; createCorner(b1,6)
            b1.MouseButton1Click:Connect(function() SaveToFile(CurrentConfigName, GetCurrentState()); ClosePopup() end)
            local b2 = Instance.new("TextButton"); b2.Size=UDim2.new(1,0,0,35); b2.Position=UDim2.new(0,0,0,40); b2.BackgroundColor3=Theme.Bg; b2.Text="Save as New"; b2.TextColor3=Theme.Text; b2.Parent=c; createCorner(b2,6)
            b2.MouseButton1Click:Connect(function()
                ClosePopup()
                ShowPopup("NEW CONFIG", function(c2)
                    local box = Instance.new("TextBox"); box.Size=UDim2.new(1,0,0,35); box.BackgroundColor3=Theme.Element; box.Text=""; box.PlaceholderText="Enter Name..."; box.TextColor3=Theme.Text; box.Parent=c2; createCorner(box,6)
                    local confirm = Instance.new("TextButton"); confirm.Size=UDim2.new(1,0,0,35); confirm.Position=UDim2.new(0,0,0,40); confirm.BackgroundColor3=Theme.Green; confirm.Text="CREATE"; confirm.TextColor3=Theme.Bg; confirm.Parent=c2; createCorner(confirm,6)
        ...
        ShowPopup("NEW CONFIG", function(c)
            local box = Instance.new("TextBox"); box.Size=UDim2.new(1,0,0,35); box.BackgroundColor3=Theme.Element; box.Text=""; box.PlaceholderText="Enter Name..."; box.TextColor3=Theme.Text; box.Parent=c; createCorner(box,6)
            local confirm = Instance.new("TextButton"); confirm.Size=UDim2.new(1,0,0,35); confirm.Position=UDim2.new(0,0,0,40); confirm.BackgroundColor3=Theme.Green; confirm.Text="CREATE"; confirm.TextColor3=Theme.Bg; confirm.Parent=c; createCorner(confirm,6)
            confirm.MouseButton1Click:Connect(function() if box.Text~="" then SaveToFile(box.Text, GetCurrentState()); ClosePopup() end end)
            return 80
        end)
    end
...
local LoadBtn = mkTool("LOAD CONFIG", Theme.Blue, function()
    ShowPopup("SELECT CONFIG", function(container)
        local scroll = Instance.new("ScrollingFrame"); scroll.Size=UDim2.new(1,0,0,150); scroll.BackgroundTransparency=1; scroll.Parent=container; scroll.ScrollBarThickness=3
        local layout = Instance.new("UIListLayout"); layout.Parent=scroll; layout.Padding=UDim.new(0,2)
        local count = 0
        if isfile(FileName) then
            local r = readfile(FileName)
            local all = HttpService:JSONDecode(r)
            for name, _ in pairs(all) do
                if name ~= "LastUsed" then
                    count = count + 1
                    local b = Instance.new("TextButton"); b.Size=UDim2.new(1,0,0,30); b.BackgroundColor3=Theme.Bg; b.Text=name; b.TextColor3=Theme.SubText; b.Parent=scroll; createCorner(b,6)
                    b.MouseButton1Click:Connect(function()
                        ClosePopup()
                        ShowPopup("MANAGE: "..name, function(c2)
                            local l = Instance.new("TextButton"); l.Size=UDim2.new(1,0,0,30); l.BackgroundColor3=Theme.Green; l.Text="LOAD"; l.TextColor3=Theme.Bg; l.Parent=c2; createCorner(l,6)
                            l.MouseButton1Click:Connect(function() LoadSpecific(name); ClosePopup() end)
                            local r = Instance.new("TextButton"); r.Size=UDim2.new(1,0,0,30); r.Position=UDim2.new(0,0,0,35); r.BackgroundColor3=Theme.Blue; r.Text="RENAME"; r.TextColor3=Theme.Bg; r.Parent=c2; createCorner(r,6)
                            r.MouseButton1Click:Connect(function()
                                ClosePopup()
                                ShowPopup("RENAME TO...", function(c3)
                                    local box = Instance.new("TextBox"); box.Size=UDim2.new(1,0,0,35); box.BackgroundColor3=Theme.Element; box.Text=""; box.PlaceholderText="New Name..."; box.TextColor3=Theme.Text; box.Parent=c3; createCorner(box,6)
                                    local confirm = Instance.new("TextButton"); confirm.Size=UDim2.new(1,0,0,35); confirm.Position=UDim2.new(0,0,0,40); confirm.BackgroundColor3=Theme.Blue; confirm.Text="UPDATE"; confirm.TextColor3=Theme.Bg; confirm.Parent=c3; createCorner(confirm,6)
                                    confirm.MouseButton1Click:Connect(function()
                                        if box.Text ~= "" and box.Text ~= name then
                                            local f = readfile(FileName); local d = HttpService:JSONDecode(f)
                                            d[box.Text] = d[name]; d[name] = nil
                                            if d["LastUsed"] == name then d["LastUsed"] = box.Text end
                                            writefile(FileName, HttpService:JSONEncode(d)); ClosePopup(); ShowNotification("Renamed!", Theme.Blue)
                                        end
                                    end)
                                    return 80
                                end)
                            end)
                            local d = Instance.new("TextButton"); d.Size=UDim2.new(1,0,0,30); d.Position=UDim2.new(0,0,0,70); d.BackgroundColor3=Theme.Red; d.Text="DELETE"; d.TextColor3=Theme.Bg; d.Parent=c2; createCorner(d,6)
                            d.MouseButton1Click:Connect(function() 
                                local f = readfile(FileName); local d = HttpService:JSONDecode(f)
                                d[name] = nil; if d["LastUsed"]==name then d["LastUsed"]=nil end
                                writefile(FileName, HttpService:JSONEncode(d)); ClosePopup(); ShowNotification("Deleted", Theme.Red)
                            end)
                            return 105
                        end)
                    end)
...
end, P_Sys); LoadBtn.Size=UDim2.new(0.9,0,0,45)
...
local ResetBtn = mkTool("RESET CONFIG", Theme.Red, function()
    ...
    AreButtonsHidden = false; HideBtn.Text = "HIDE BUTTONS: OFF"; HideBtn.BackgroundColor3 = Theme.Red
    UpdateTransparencyFunc()
    IsJoystickEnabled = false; JoyContainer.Visible = false; JoyToggle.Text="JOYSTICK: OFF"; JoyToggle.BackgroundColor3=Theme.Red
    IsLayoutLocked = false; JoyContainer.Position = UDim2.new(0.1, 0, 0.6, 0)
    CurrentComboIndex = 0; CurrentConfigName = nil; SkillMode = "INSTANT"
    
    if ResizerUpdateFunc then ResizerUpdateFunc() end
    updateLockState()
    RefreshControlUI()
    ShowNotification("Config Reset!", Theme.Accent)
end, P_Sys); ResetBtn.Size=UDim2.new(0.9,0,0,45)

local ExitBtn = mkTool("EXIT SCRIPT", Theme.Red, function() isRunning = false; ScreenGui:Destroy() end, P_Sys); ExitBtn.Size=UDim2.new(0.9,0,0,45)
...
task.delay(0.5, function()
    if isfile(FileName) then
        ...
        if s then
            ...
            if data["LastUsed"] and data[data["LastUsed"]] then LoadSpecific(data["LastUsed"]) end
        end
    end
end)
